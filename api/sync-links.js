...
// The 5 Calendar IDs you are monitoring
const CALENDAR_IDS = [
...
async function scrapeSquarespaceLinks() {
  try {
    ...
    while ((match = linkRegex.exec(html)) !== null) {
      ...
      // Clean up the URL slug to use as a name for matching
...
export default async function handler(req, res) {
  // --- EASY TESTING LOGIC ---
  // This allows you to test by visiting: /api/sync-links?key=your_password
  const providedSecret = req.query.key || req.headers.authorization?.split(' ')[1];
  
  if (!process.env.CRON_SECRET || providedSecret !== process.env.CRON_SECRET) {
    return res.status(401).json({ 
      success: false, 
      message: 'Unauthorized. Please use ?key=YOUR_SECRET in the URL.' 
    });
  }
  ...
  try {
    ...
    for (const calendarId of CALENDAR_IDS) {
      ...
      for (const event of (events.data.items || [])) {
        // Try to find a match between the website links and the calendar title
        ...
        // Update if a match is found and the link isn't already in the description
    ...
    return res.status(200).json({ 
      success: true, 
      message: `Sync complete. Updated ${updatedCount} events.`,
      linksFoundOnSite: siteLinks.length 
    });
